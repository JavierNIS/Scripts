...
#example: HOW_MANY_TARGETS_ARE_BUILT["static"] = number_of_static_libraries
...
verify_parameters(){
    ...
    if [ "$#" -eq 0 ]; then FINISHED=1; break; fi
      ...
      if [ "$param" == "$1" ]; then
        ...
        HOW_MANY_TARGETS_ARE_BUILT["dynamic"]=$2
				shift 2 
				PARAMS_USED=$((PARAMS_USED+2))
			fi
		;;
        ...
        HOW_MANY_TARGETS_ARE_BUILT["static"]=$2
				shift 2
				PARAMS_USED=$((PARAMS_USED+2))
			fi
		;;
...
populate_makefile(){
  ...
  #Changed the way the naming is made, types is a helper array
  #that will store the type of each target
  local names_bin=()
  local names_static=()
  local names_dynamic=()
  local types=()
  local number
  for t in "${ORDER_IN_WHICH_TO_BUILD[@]}"; do 
    number=${HOW_MANY_TARGETS_ARE_BUILT[$t]:-0}
    for ((i=0;i<number;i++)); do 
      types+=("$t")
    done
  done
  
  #the rest of the targets are considered binaries
  while (( ${#types[@]} < ${#TARGET_FILES[@]} )); do 
    types+=("binary")
  done

  #now we can safely get the names of the targets
  local type
  local target_base
  for i in "${!TARGET_FILES[@]}"; do 
    type="${types[$i]}"
    target_base="${TARGET_FILES[$i]%.*}"
    case "$type" in 
      static) names_static+=("$target_base.ar") ;;
      dynamic) names_dynamic+=("$target_base.so") ;;
      binary) names_bin+=("$target_base") ;;
      *) printf "Error while populating makefile with targets, exiting..."; exit 1 ;;
    esac
  done
  ...
  local all_final_targets=()
  #Get targets
  if (( ${#names_bin[@]} > 0 )); then
    printf 'BINARIES=' >> $MAKE_NAME
    for name in "${names_bin[@]}"; do 
      printf ' %s' "$name" >> $MAKE_NAME
    done
    printf "\n" >> $MAKE_NAME
    printf 'BINARIES_PATH := $(addprefix $(BIN_DIR)/,$(BINARIES))\n' >> $MAKE_NAME
    all_final_targets+=('$(BINARIES_PATH)')
  fi
  
  if (( ${#names_static[@]} > 0 )); then
    printf 'STATIC_LIBS=' >> $MAKE_NAME
    for name in "${names_static[@]}"; do 
      printf ' %s' "$name" >> $MAKE_NAME
    done
    printf "\n" >> $MAKE_NAME
    printf 'STATIC_PATH := $(addprefix $(BIN_DIR)/lib/,$(STATIC_LIBS))\n' >> $MAKE_NAME
  ...
  if (( ${#names_dynamicc[@]} > 0 )); then
    ...
    printf "\n" >> $MAKE_NAME
    printf 'DYNAMIC_PATH := $(addprefix $(BIN_DIR)/lib/,$(DYNAMIC_LIBS))\n' >> $MAKE_NAME
    all_final_targets+=('$(DYNAMIC_PATH)')
  fi
  ...
  printf 'all: dirs' >> $MAKE_NAME
  for target in "${all_final_targets[@]}"; do 
    printf ' %s' "$target" >> $MAKE_NAME
  done
  printf '\n' >> $MAKE_NAME
  ...
  if [ ! "${#names_bin[@]}" -eq 0 ]; then
    printf "#Link it all together\n" >> $MAKE_NAME
    printf '$(BIN_DIR)/%%: $(OBJ_DIR)/%%.o $(SOURCES_NT_OBJ)\n' >> $MAKE_NAME
    printf '\t$(CXX) $(CXXFLAGS) -o $@ $^\n\n' >> $MAKE_NAME
  fi
  if [ ! "${#names_static[@]}" -eq 0 ]; then
    printf "#Make the static libraries\n" >> $MAKE_NAME
    printf '$(BIN_DIR)/lib/%%.ar: $(OBJ_DIR)/%%.o $(SOURCES_NT_OBJ)\n' >> $MAKE_NAME
    printf '\tar rcs $@ $^\n\n' >> $MAKE_NAME

  fi
  if [ ! "${#names_dynamic[@]}" -eq 0 ]; then
    printf "#Make the dynamic libraries\n" >> $MAKE_NAME
    printf '$(BIN_DIR)/lib/%%.so: $(OBJ_DIR)/%%.o $(SOURCES_NT_OBJ)\n' >> $MAKE_NAME
    printf '\t$(CXX) $(CXXFLAGS) -shared -o $@ $^\n\n' >> $MAKE_NAME
  fi
